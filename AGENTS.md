...
One repo-specific exception, decided deliberately: the replacements editor
keeps its draft-plus-Save semantics inside a `render:` row. Replacement keys
are live the moment they are stored — persisting every keystroke would let a
half-typed key match and expand text in notes while the user is still typing
it. Edits stay local until the Save button commits the validated list; do NOT
convert it to immediate per-change writes.
