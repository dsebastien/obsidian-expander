 ...
 *
 * Pattern breakdown:
 * - <!--\s*(expander(?:-(?:manual|once(?:-and-eject)?))?):\s* - Opening marker with variant
 * - ([a-z0-9]+(?:-[a-z0-9]+)*) - Key in kebab-case
 * - \s*--> - End of opening marker
 * - ([\s\S]*?) - Value content (non-greedy, including newlines)
 * - <!--\s*\/(expander(?:-(?:manual|once(?:-and-eject)?))?):\s* - Closing marker with variant
 * - ([a-z0-9]+(?:-[a-z0-9]+)*) - Closing key for validation
 * - \s*--> - End of closing marker
 */
const EXPANDER_REGEX =
    /<!--\s*(expander(?:-(?:manual|once(?:-and-eject)?))?):\s*([a-z0-9]+(?:-[a-z0-9]+)*)\s*-->([\s\S]*?)<!--\s*\/(expander(?:-(?:manual|once(?:-and-eject)?))?):\s*([a-z0-9]+(?:-[a-z0-9]+)*)\s*-->/g
...
function markerToMode(marker: string): UpdateMode {
    switch (marker) {
        case 'expander':
            return 'auto'
        case 'expander-manual':
            return 'manual'
        case 'expander-once':
            return 'once'
        case 'expander-once-and-eject':
